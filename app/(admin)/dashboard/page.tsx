...
function StatCard({ label, value, icon, color, sub, delay }: {
  ...
  return (
    <div style={{
      ...
      className="stat-card-hover animate-fade-up"
    >
      <div style={{
...
function FunnelBar({ label, pct, count, color, delay }: {
  ...
  return (
    <div style={{ marginBottom: 16, animationDelay: `${delay}ms` }} className="animate-fade-up">
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13 }}>
        <span style={{ fontWeight: 600, color: 'var(--text2)' }}>{label}</span>
        <span style={{ fontWeight: 800, color }}>{count}명 · {pct}%</span>
      </div>
      <div style={{ height: 10, background: 'var(--bg3)', borderRadius: 100, overflow: 'hidden', position: 'relative' }}>
        <div style={{
...
function LeadRow({ lead, idx }: { lead: Lead; idx: number }) {
  const colors: Record<string, string> = { new: '#6c63ff', contact: '#f59e0b', done: '#22c55e', trash: '#ef4444' }
  const c = colors[lead.status]
  return (
    <div style={{
  ...
  return (
    <>
      <style>{`
        ...
        .action-btn:hover { transform: translateY(-3px); border-color: var(--accent) !important; box-shadow: 0 8px 24px var(--accent-glow); }
        ...
        .wave-wrap { position:absolute;bottom:0;left:0;right:0;overflow:hidden;line-height:0;pointer-events:none; }
        ...
        .particle { position:absolute;animation:particleFloat var(--dur) ease-in-out infinite alternate;pointer-events:none; }
        ...
        .pulseDot { animation:pulseDot 2s infinite; }
        ...
        @media(max-width:480px){
          ...
          .actions-inner{grid-template-columns:repeat(2,1fr)!important}
      ...
      <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)' }}>
        ...
        <div style={{
          ...
          <div style={{ position: 'relative', zIndex: 1, maxWidth: 1400, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            <div>
              ...
              <h1 style={{ fontSize: 'clamp(22px,4vw,34px)', fontWeight: 900, letterSpacing: -1, lineHeight: 1.2, marginBottom: 6 }}>
          ...
          <div className="wave-wrap">
            <svg className="wave-svg" viewBox="0 0 1440 60" style={{ display: 'block' }}>
          ...
          <div className="wave-wrap" style={{ bottom: 0 }}>
            <svg viewBox="0 0 1440 40" style={{ display: 'block', animationDelay: '1s' }} className="wave-svg">
              <path fill="#f472b6" fillOpacity="0.06" d="M0,20 C240,40 480,0 720,20 C960,40 1200,0 1440,20 L1440,40 L0,40 Z" />
            </svg>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1400, margin: '0 auto', padding: '0 20px 100px' }}>
          ...
          {/* STATS */}
          <div className="stats-grid-inner" style={{
            display: 'grid', gridTemplateColumns: 'repeat(4,1fr)',
            gap: 14, marginTop: -40, marginBottom: 24, position: 'relative', zIndex: 2,
          }}>
          ...
          {/* QUICK ACTIONS */}
          <div className="actions-inner" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 20 }}>
            {[
              { href: '/builder', icon: '✨', label: '랜딩 만들기', color: '#6c63ff' },
              { href: '/leads', icon: '👥', label: '리드 목록', color: '#22c55e' },
              { href: '/messages', icon: '💬', label: '메시지', color: '#f59e0b' },
              { href: '/settings', icon: '⚙️', label: '설정', color: '#f472b6' },
            ].map((a, i) => (
              <Link key={a.href} href={a.href} className="action-btn" style={{
                display: 'flex', flexDirection: 'column', alignItems: 'center',
                gap: 8, padding: '16px 12px',
                background: 'var(--surface)', border: '1px solid var(--border)',
                borderRadius: 16, textDecoration: 'none', color: 'var(--text)',
                transition: 'all 0.2s', animationDelay: `${i * 60}ms`,
              }}>
                <span className="float-icon" style={{ fontSize: 28, animationDelay: `${i * 0.3}s`, filter: `drop-shadow(0 4px 8px ${a.color}66)` }}>{a.icon}</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 18, alignItems: 'start' }}>
            <div>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 20, marginBottom: 18 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, fontSize: 15, fontWeight: 800 }}>
                  <div className="pulseDot" style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--accent)', boxShadow: '0 0 8px var(--accent)' }} />
                  전환 퍼널
                  <span style={{ fontSize: 12, color: 'var(--text3)', marginLeft: 'auto', fontWeight: 500 }}>실시간</span>
                </div>
                {loading ? [1,2,3].map(i => <div key={i} className="skeleton" style={{ height: 48, marginBottom: 14 }} />) : funnelData.map((f, i) => <FunnelBar key={f.label} {...f} delay={i * 150} />)}
              </div>
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 15, fontWeight: 800 }}>
                    <div className="pulseDot" style={{ width: 8, height: 8, borderRadius: '50%', background: '#f472b6', boxShadow: '0 0 8px #f472b6' }} />
                    최근 신청자
                  </div>
            ...
            <div className="pc-aside" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 18, padding: '24px 16px', textAlign: 'center' }}>
                <div className="char-float" style={{ fontSize: 64 }}>🚀</div>
